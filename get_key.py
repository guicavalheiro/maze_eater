...
try:
    ...
    def _get_key():
        ...
        if ord(char) == 224:
            ...
            #print(bytes("oi", 'utf-8'))
            return char.decode('utf-8')

    def simulate_press(key):
        
        if key == 'up' or key == 'down' or key == 'left' or key == 'right':
            return simulate_keys[key]    
        else:
            key = bytes(key, 'utf-8')
...
def simulate_key(key_sim):
    
    while True:
        key = simulate_press(key_sim)
        #print(f"key: {key}")
        if key in hotkeys:
            hotkeys[key]()
            continue
...
if __name__ == '__main__':
    ...
    simulate_list = ['up', 'up', 'down', 'q']
    
    while True:
        for key_sim in simulate_list: 
            key = simulate_key(key_sim)
            print(key)
            if key == 'q':
                exit()
